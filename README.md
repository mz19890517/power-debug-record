...
- **云端项目同步修复（v2.29，关键）**：修复「其他设备只同步到柜子类型、拉不到项目数据」——WebDAV 集合（子目录）href 按 RFC 4918 以 `/` 结尾，旧版 `substringAfterLast('/')` 会把子目录名整个解析成空串，导致**任何设备都枚举不到 `projects/` 下的项目文件夹**（上传正常、下载全断，v2.26~v2.28 共有的致命缺陷）。改为正确保留目录尾斜杠后再识名，多设备增量互通恢复
