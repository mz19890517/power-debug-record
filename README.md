...
- **按项目增量同步（v2.26，★架构升级）**：云端改为「**全局区 + 每项目一个文件夹**」新结构，同步只搬运有改动的数据（规格第 7 章）：
  - 云端布局：`global/backup_<账号>_<本机标识>.json`（柜子类型、候选池、调试员名单、**全部删除墓碑含项目级**）与 `projects/project_<项目名>/backup_<账号>_<本机标识>.json`（该项目全部柜子/日志/故障/预选项 + 项目行）；**v2.29 起项目文件夹名=清洗后的项目名**（可读好找），重名项目自动带 id 短缀，项目改名/旧版 `project_<UUID>` 文件夹仍会被解析合并、云端照常收敛
...
- **云端文件夹以项目名为准（v2.29）**：云端 `projects/project_<项目名>/` 文件夹直接显示项目名（清洗掉 `/`、`\`、`:`、`*`、`?`、`"`、`<`、`>`、`|` 等非法字符，超长截断64），**坚果云里一眼找到对应项目**；重名项目自动加 id 短缀避免文件夹相互覆盖；项目改名/旧版 `project_<UUID>` 文件夹仍被按快照内项目 id 解析合并，改名后同步自然收敛到新名字文件夹
