...
- Room schema 当前 version=12（迁移链 1→2→3→…→12 必须保持完整，禁止 fallbackToDestructiveMigration）
- JSON 备份 schemaVersion=12，字段名=数据库列名，是将来 PC/网页端的交换格式；v2.30 起 Project 快照含 debugStartDate/debugEndDate（调试起始/完成日期，旧备份缺键按 0 兼容解析）；v2.27 起 DebugLog 快照含 logType（0通过/1故障/2消除），旧备份缺键按 0 兼容解析；v2.10 起快照经 gzip 压缩上传，读取按魔数(1f 8b)自动兼容明文旧格式（util/WebDavSync.decodeSnapshot，公开可复用）；快照头含 deviceTimeZoneOffset；readyKind=full/global/project 区分全量/全局区/单项目三种快照
...
- ⚠ v2.29 云端项目枚举修复（两段，勿再破坏）：WebDAV 集合 href 理论上按 RFC 4918 必带尾斜杠，但实测坚果云对部分集合不补尾斜杠——两处都必须宽容，改任何一处必回归对应测试：① core/WebDavClient.kt `parseHrefChild`：旧版 substringAfterLast('/') 把目录名吞空 → projects/ 恒空、任何设备拉不到项目（只同步柜子类型）；保留目录尾斜杠、文件纯名、URL解码（回归 WebDavClientHrefTest）。② util/WebDavSync.kt `projectKeyOf`：枚举不再依赖 namesWithSlash 假设，带/不带尾斜杠都归一出 project_<UUID>，过滤非 project_ 前缀/空名/深层路径（回归 WebDavSyncProjectKeyTest）。listBackups 跳过目录项
