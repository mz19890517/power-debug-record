...
- ⚠ v2.29 云端目录枚举修复：WebDAV 集合 href 必带尾斜杠（RFC 4918），core/WebDavClient.kt 旧版 substringAfterLast('/') 会把 projects/ 子目录名吞成空串 → 任何设备都拉不到项目数据（只同步柜子类型）。已改 parseHrefChild 保留目录尾斜杠、文件纯名；listChildren 返回子目录仍带 "/"（WebDavSync 枚举靠它区分），listBackups 跳过目录项。改目录枚举/解析时务必回归 WebDavClientHrefTest
